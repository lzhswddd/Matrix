...
double nn::mSum(const Matrix & src)
{
	if (src.empty()) {
		cerr << errinfo[ERR_INFO_EMPTY] << endl;
		throw src;
...
const Matrix nn::range(int low, int top, double gap)
{
	if (low >= top) {
		cerr << errinfo[ERR_INFO_VALUE] << endl;
		throw errinfo[0];
	}
	int len = int(abs(low - top) / gap) + 1;
	Matrix mark(1, len);
	mark(0) = low;
...
const Matrix nn::range(double low, double top, double gap)
{
	if (low >= top) {
		cerr << errinfo[ERR_INFO_VALUE] << endl;
		throw errinfo[0];
...
const Matrix nn::mRand(int low, int top, Size size, bool isdouble)
{
	return mRand(low, top, size.hei, size.wid, 1, isdouble);
}
const Matrix nn::mRand(int low, int top, Size3 size, bool isdouble)
{
	return mRand(low, top, size.y, size.x, size.z, isdouble);
}
...
const Matrix nn::mPow(const Matrix &temp, double num)
{
	if (temp.empty()) {
		cerr << errinfo[ERR_INFO_EMPTY] << endl;
		throw temp;
